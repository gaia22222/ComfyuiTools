...
頁面提供 `女性乳頭`、`陰莖整體`、`陰部` 三個自動偵測開關。未勾選的類別不會建立黃色模型框；取消勾選亦會移除該類別現有的黃色框，但保留所有粉紅色人手框。開關狀態會與打碼效果、強度及擴張設定一同保存於瀏覽器。

「一鍵偵測及打碼全部圖片」會依序處理已導入的全部圖片，沿用同一組類別開關及效果設定，保留粉紅色人手框、取代舊黃色模型框，並直接更新每張圖片的打碼預覽。進度列會顯示目前圖片及完成數量；使用者可要求在完成目前圖片後停止。單張圖片失敗時，其餘圖片仍會繼續處理。
